...
function SetDbState(server,name,state)
{
  /*  
  ...
  $command=new-object system.Data.SqlClient.SqlCommand('ALTER DATABASE "{name}" SET SINGLE_USER WITH ROLLBACK IMMEDIATE;ALTER DATABASE "{name}" SET {state};ALTER DATABASE "{name}" SET MULTI_USER;',$connection);
  $connection.Open();
