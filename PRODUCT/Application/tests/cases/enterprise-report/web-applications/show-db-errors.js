module('when error happens', {
  ...
  setup: function() {
    ...
    with(new PowerShell(context.host)) {
      $.when( invokeLambda(SetDbState, 'sp-2k10-u1','WSS_Content_41601',"OFFLINE") ).then(function(){
        start();
      });
    };
  }, 
  teardown: function() {
    this.report_url = context.url + '/' + context.report_url;
    with(new PowerShell(context.host)) {
      $.when( invokeLambda(SetDbState, 'sp-2k10-u1','WSS_Content_41601',"ONLINE") ).then(function(){
          start();
      });
    }  
  }
