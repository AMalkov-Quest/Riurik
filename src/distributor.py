...
def makeSaveContentPost(content, path):
	...
	return src.contrib.convert_dict_values_strings_to_unicode(data)
...
def saveToolsAllScripts(url, document_root, virtual_root, ctx):
	tools_dirs = src.contrib.get_context_tools_folders(ctx)
	for tools_dir in tools_dirs:
		log.info('save tools folder: %s' % tools_dir)
		virtual_path = os.path.join(virtual_root, tools_dir)
		tools_dir_fullpath = src.contrib.get_full_path(document_root, virtual_path)
		files = src.contrib.enum_files_in_folders(tools_dir_fullpath)
...
def uploadFolderToRemote(folder_content, virtual_path, document_root, ctx, url, name):
	for file_ in folder_content:
		...
		rel_path = src.contrib.get_relative_clean_path(file_path)
		fullpath = src.contrib.get_full_path(document_root, file_path)
...
def saveAllLibraries(document_root, virtual_root, url, path, ctx):
	for lib in src.contrib.get_libraries(path, ctx):
		lib_relpath = src.contrib.get_lib_path_by_name(document_root, lib, ctx)
		if lib_relpath:
			lib_path = os.path.join(virtual_root, lib_relpath)
			fullpath = src.contrib.get_full_path(document_root, lib_path)
...
def saveTestSatelliteScripts(url, path, ctx):
	...
	"""
	document_root = src.contrib.get_document_root(path)
	virtual_root = src.contrib.get_virtual_root(path)
...
def saveSuiteAllTests(url, path, ctx):
	document_root = src.contrib.get_document_root(path)
	tests = src.contrib.enum_suite_tests( src.contrib.get_full_path(document_root, path) )
