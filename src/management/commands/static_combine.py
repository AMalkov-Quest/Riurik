...
from django.conf import src.settings

from plugins.static_management.lib import static_combine, get_version, write_versions

try:
    CSS_ASSET_PATTERN = re.compile(src.settings.STATIC_MANAGEMENT_CSS_ASSET_PATTERN)
...
class Command(BaseCommand):
    ...
    def combine_js(self):
        try:
            js_files = src.settings.STATIC_MANAGEMENT['js']
        except AttributeError:
    ...
    def combine_css(self):
        try:
            css_files = src.settings.STATIC_MANAGEMENT['css']
        except AttributeError:
    ...
    def replace_css(self, filename):
        tmp = os.tmpfile()
        rel_filename = os.path.join(src.settings.MEDIA_ROOT, filename)
        ...
        for line in css:
            ...
            for match in re.finditer(CSS_ASSET_PATTERN, line):
                try:
                    ...
                    if absolute:
                        asset_path = os.path.join(src.settings.MEDIA_ROOT, '.'+grp['filename'])
                    else:
                        asset_path = os.path.join(os.path.dirname(rel_filename), grp['filename'])
                    asset = relpath(asset_path, src.settings.MEDIA_ROOT)
    ...
    def find_assets(self):
        if src.settings.STATIC_MANAGEMENT_ASSET_PATHS:
            exp = re.compile(src.settings.STATIC_MANAGEMENT_ASSET_PATTERN)
            for path, recurse in src.settings.STATIC_MANAGEMENT_ASSET_PATHS:
                if recurse:
                    for root, dirs, files in os.walk(os.path.join(src.settings.MEDIA_ROOT, path)):
                        for filename in files:
                            if exp.match(filename):
                                yield relpath(os.path.join(root, filename), src.settings.MEDIA_ROOT)
                else:
                    for filename in os.listdir(os.path.join(src.settings.MEDIA_ROOT, path)):
                        full_filename = os.path.join(src.settings.MEDIA_ROOT, os.path.join(path, filename))
                        if not os.path.isdir(full_filename):
                            if exp.match(filename):
                                yield relpath(full_filename, src.settings.MEDIA_ROOT)

    def get_versions(self, css_only=False):
        hosts = src.settings.STATIC_MANAGEMENT_HOSTNAMES
        i = 0
        if css_only:
        ...
        for main_file in files:
            if i > len(hosts) - 1:
                i = 0
            version = get_version(os.path.join(src.settings.MEDIA_ROOT, main_file), main_file, src.settings.STATIC_MANAGEMENT_VERSIONER)
    ...
    def write_versions(self):
        for main_file in self.files_created:
            if self.options['write-version']:
                shutil.copy2(os.path.join(src.settings.MEDIA_ROOT, main_file),
                             os.path.join(src.settings.MEDIA_ROOT, self.versions[main_file]))
        if self.options['output']:
            write_versions(self.abs_versions, src.settings.STATIC_MANAGEMENT_VERSION_WRITER)

def combine_files(files, options):
    for main_file in files:
        # create file
        main_file_path = os.path.join(src.settings.MEDIA_ROOT, main_file)
        # go and get sub files
        to_combine = recurse_files(main_file, files[main_file], files)
        to_combine_paths = [os.path.join(src.settings.MEDIA_ROOT, f_name) for f_name in to_combine if os.path.exists(os.path.join(src.settings.MEDIA_ROOT, f_name))]
        static_combine(os.path.join(src.settings.MEDIA_ROOT, main_file), to_combine_paths, compress=options['compress'])
